<template>
  ...
  >
    ...
    <div class="message__text">
      <span class="message__text-content">{{ decryptedText }}</span><div class="message__footer-place">
...
import { type Message, type CurrentChat } from '@/stores/chat'
...
const decryptedText = computed(() => {
  return resolveMessageText(props.message, {
    ...
    encryptedFallback: '[Защищенное сообщение]',
...
const replyPreviewText = computed(() => {
  if (!props.message.replyTo) {
    return 'Исходное сообщение было удалено или еще не загружено'
  }
...
function onContextMenu(e: MouseEvent) {
  emit('context-menu', { mouseEvent: e, message: props.message })
}
